...
def finetune(args, tokenizer: AutoTokenizer, model: deepspeed.DeepSpeedEngine, optimizer: AdamW, lr_scheduler, dataset, device, teacher_model=None, projectors=None):
    print_rank("Start Fine-tuning")

    use_span_loss = projectors is not None and getattr(args, "w_span_loss", 0) > 0
    if use_span_loss:
    ...
    for epoch in range(args.epochs):
        ...
        for it, (model_batch, no_model_batch, gen_data) in enumerate(train_dataloader):
            ...
            outputs = model(**model_batch, output_hidden_states=use_span_loss, use_cache=False)
            ...
            if teacher_model is not None:
                if use_span_loss:
                    ...
                    span_loss = compute_overall_span_loss(projectors, model_batch['attention_mask'], logits, teacher_logits,
...
def main():
    ...
    if args.do_train:
        model = finetune(args, tokenizer, model, optimizer, lr_scheduler, dataset, device, teacher_model=teacher_model, projectors=projector_list)
